<template>
  <template v-if="hasStandings">
    <div>
      <template v-for="leagueGroup in groupedStandings" :key="leagueGroup.leagueId">
        <!-- League with divisions -->
        <template v-if="leagueGroup.divisions">
          <template v-for="division in leagueGroup.divisions" :key="division.division">
            <table class="table table-fixed align-middle text-center">
              <thead class="sticky-thead">
                <tr>
                  <th class="text-start">{{ division.division }}</th>
                  <th v-for="col in sortableColumns" :key="col.key">{{ col.label }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="team in division.teams" :key="team.team_id">
                  <td class="text-start">
                    <div class="d-flex align-items-center gap-2">
                      <span :class="team.pos <= 4 ? 'badge bg-primary' : 'badge bg-secondary'">
                        {{ team.pos }}
                      </span>
                      <img
