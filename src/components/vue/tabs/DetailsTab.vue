    ...
    <div class="list-group list-group-flush">
      ...
      <div class="list-group-item">
        ...
        <div v-for="stat in perGameStats" :key="stat.key" class="mb-3">
          <div class="d-flex justify-content-between mb-1">
            <small>{{ stat.team1.toFixed(1) }} <small class="text-muted">({{ getOrdinalSuffix(stat.team1Rank) }})</small></small>
            <small>{{ stat.label }}</small>
            <small><small class="text-muted">({{ getOrdinalSuffix(stat.team2Rank) }})</small> {{ stat.team2.toFixed(1) }}</small>
          </div>
          <div class="progress-stacked">
            <div
              class="progress"
...
const home = computed(() => homeId.value ? props.clubs[homeId.value] : undefined);
const away = computed(() => awayId.value ? props.clubs[awayId.value] : undefined);
const league_name = computed(() => leagueId.value ? props.leagues[leagueId.value]?.name ?? '' : '');

const team1 = computed(() => {
  if (!leagueId.value || !homeId.value) return undefined;
  return props.standings[leagueId.value]?.find(s => s.team_id === homeId.value);
});

const team2 = computed(() => {
  if (!leagueId.value || !awayId.value) return undefined;
  return props.standings[leagueId.value]?.find(s => s.team_id === awayId.value);
});
...
const perGameStats = computed(() => {
  ...
  const leagueStandings = leagueId.value ? props.standings[leagueId.value] : undefined;

  return keys.map(({ key, label }) => {
    const team1Value = team1.value ? perGame(team1.value, key) : 0;
    const team2Value = team2.value ? perGame(team2.value, key) : 0;

    const team1Rank = team1.value ? getPerGameRank(leagueStandings, team1.value.team_id, key) : null;
    const team2Rank = team2.value ? getPerGameRank(leagueStandings, team2.value.team_id, key) : null;
